...
exports.createTransaction = function(data, fnCallback) {
  ...
  db.collection(TBN_TRANSACTION).insert(data, function(err, results) {
 ...
 * @rows business documents identifiers, form as: [{collection:'<collection name>', id:'<doc id>'}, ...]
 */
exports.commit = function(transId, rows, fnCallback) {  
  console.log('[T] Commit transaction: %j', transId);

  changeTransState(transId, 'committed', function(trans) {

    exports.unbindWithTransaction(transId, rows, function(result) {
      if(result) {
 ...
 * 
 */
exports.unbindWithTransaction = function(transId, rows, fnCallback) {
  console.log('[T] Unbind with transaction %j', rows);
  var n = 0;
  var fail = false;
  for(var i=0;i<rows.length;i++) {
    ...
    db.collection(tb).findAndModify({_id:db.toObjectID(id)}, QUERY_SINGLE_ORDER , {$pull:{pendingTransactions:transId}}, {safe:true, 'new':true}, function(err, result) {  
      if(err || !result) {
        // 无需处理错误，交给恢复程序处理。
...
function changeTransState(transId, newState, fnCallback) {
  db.collection(TBN_TRANSACTION).findAndModify({_id:db.toObjectID(transId)}, QUERY_SINGLE_ORDER, {$set:{state:newState}}, {safe:true, 'new':true}, function(err, result) {  
    if(err || !result) {
...
exports.recoverPendingTransactions = function(fnCallbackPending) {
  ...
  db.collection(TBN_TRANSACTION).findItems({state:{$in:['initial', 'pending']}}, {safe:true}, function(err, count, transs) {
    if(err) {
...
exports.recoverCommittedTransactions = function(fnCallbackCommitted) {
  ...
  db.collection(TBN_TRANSACTION).findItems({state:'committed'}, function(err, transs) {
    if(err) {
      console.log('[T] Fail to recover: %j', util.inspect(err));
